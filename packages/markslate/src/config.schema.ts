...
/** Single source of truth for MarkSlate settings — generates `contributes.configuration` and the typed accessor. */
export const configSchema = z.object({
	'markslate.mermaidRenderMode': setting(z.enum(['inline', 'below', 'disabled']).default('inline'), {
		markdownDescription:
	...
	'markslate.calloutDefaultTitle': setting(z.boolean().default(true), {
		markdownDescription:
	...
	'markslate.mathExportColor': setting(z.string().default('currentColor'), {
		markdownDescription:
	...
	'markslate.formatTablesOnEdit': setting(z.boolean().default(true), {
		markdownDescription:
