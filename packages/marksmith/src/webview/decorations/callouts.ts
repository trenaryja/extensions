...
const CALLOUT_HEADER_RE = /^((?:\s*>\s*)+)\[!(\w+)\]([+-]?)\s?(.*)$/i
...
function buildCalloutDecorations(state: EditorState): DecorationSet {
	...
	// --- Pass 1: find every callout (with nesting) via a depth stack ---
	...
	// Fold: outermost (`>`) callouts collapse when marked `-` and the cursor isn't inside their content.
	for (const callout of callouts)
		if (callout.depth === 1 && callout.marker === '-' && callout.to > callout.from)
			callout.collapsed = !selectionTouches(state, doc.line(callout.from + 1).from, doc.line(callout.to).to)

	// --- Pass 2: render each line with the stack of callouts it sits inside ---
	for (let n = 1; n <= doc.lines; n++) {
		...
		const levels = inside.length
		const isCollapsedHead = inside.some((c) => c.collapsed && c.from === n)

		const classes = ['md-callout-line']
		if (n === outer.from) classes.push('md-callout-line-head')
		if (n === outer.to || isCollapsedHead) classes.push('md-callout-line-last')
		let style = outer.color ? `--callout-color:${outer.color};` : ''
		if (levels > 1)
			style += `padding-left:calc(0.85em + ${levels - 1} * ${INDENT});background-image:${nestedBackground(inside)};`
		add(
			line.from,
			line.from,
			Decoration.line({ attributes: style ? { class: classes.join(' '), style } : { class: classes.join(' ') } }),
		)
		...
		if (headerHere) {
			...
			const titleStart = line.from + (line.text.length - headerHere.title.length)
			add(line.from, titleStart, hide)
			// Color a nested header's custom title (outer ones are colored/bolded via the head-line CSS).
			if (headerHere.depth > 1 && headerHere.color && headerHere.title.trim().length > 0)
				add(
					titleStart,
					line.to,
					Decoration.mark({ attributes: { style: `color:${headerHere.color};font-weight:600` } }),
				)
			if (headerHere.depth === 1 && (headerHere.marker === '+' || headerHere.marker === '-')) {
				const markerPos = line.from + (CALLOUT_HEADER_RE.exec(line.text)?.[1]?.length ?? 0) + headerHere.type.length + 3
				add(
					line.from,
					line.from,
					Decoration.widget({ widget: chevronWidget({ collapsed: !!headerHere.collapsed, pos: markerPos }), side: -1 }),
				)
			}
			add(
				line.from,
				line.from,
				Decoration.widget({ widget: iconWidget({ icon: headerHere.icon, color: headerHere.color }), side: -1 }),
			)
			if (showDefaultTitle && headerHere.title.trim().length === 0)
				add(
					line.from,
					line.from,
					Decoration.widget({
						widget: titleWidget({ text: capitalize(headerHere.type), color: headerHere.color }),
						side: -1,
					}),
				)
		} else {
