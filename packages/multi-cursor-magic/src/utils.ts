...
export type Command<TParsed, TOption extends vscode.QuickPickItem> = {
	...
	| {
			type: 'direct'
			transform: (selection: TParsed, selections: (TParsed | null)[]) => boolean | number | string
	  }
	| {
			type: 'input'
			prompt: string
			transform: (selection: TParsed, selections: (TParsed | null)[], input?: string) => boolean | number | string
	  }
	| {
			type: 'quick-pick'
			prompt: string
			quickPickItems: TOption[]
			transform: (selection: TParsed, selections: (TParsed | null)[], option: TOption) => boolean | number | string
	  }
)
